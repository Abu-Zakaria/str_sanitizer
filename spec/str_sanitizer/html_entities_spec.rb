...
RSpec.describe StrSanitizer::HtmlEntities do
  ...
  it "has some method for encoding and decoding" do
    expect(@methods.respond_to? :html_encode).to eq(true)
    expect(@methods.respond_to? :html_decode).to eq(true)
  end

  it "returns a encoded string" do
    encoded = @methods.html_encode(@encode_string)
  ...
  it "returns a decoded string" do
    decoded = @methods.html_decode(@decode_string)
  ...
  it "doesn't throw any exception if nothing was there to encode and decode" do
    ...
    expect(@methods.html_encode simple_string).to eq(simple_string)
    expect(@methods.html_decode simple_string).to eq(simple_string)
  end
end
